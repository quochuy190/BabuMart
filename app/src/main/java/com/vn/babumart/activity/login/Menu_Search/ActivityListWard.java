...
import com.vn.babumart.adapter.AdapterListWard;
...
public class ActivityListWard extends BaseActivity
    ...
    private void initData() {
        sID_DISTRICT = getIntent().getStringExtra(Constants.KEY_SEND_ID_DISTRICT);
        if (sID_DISTRICT != null && sID_DISTRICT.length() > 0) {
            showDialogLoading();
            mPresenter.api_get_ward(sID_DISTRICT);
        } else {
            finish();
        }
    }
